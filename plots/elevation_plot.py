...
sacrifice_time <- "29 day"
endpoint_col   <- "TBIL(mg/dL)"   # <-- change if TBIL is named differently
...
# Summary stats for barplot
# ================================
group_names <- c("Treatment", "", "GanCtrl")  # middle blank, we'll add Real/control
...
if (!is.finite(y_base)) {
  stop("No non-missing values for this compound / time / endpoint. ",
       "Check 'endpoint_col' and data for ethanol, 29 day, TBIL.")
}

# ---- Fixed y-axis limit ----
ylim_top <- 0.50

# Place brackets at reasonable heights within [0, 0.40]
y1 <- ylim_top * 0.68    # Treatment vs Real control bracket height
y2 <- ylim_top * 0.77    # Treatment vs SynCtrl bracket height
...
# ================================
tiff(
  filename = "/account001/mansi.chandra/clin_path/results_vae_corr_mod3_cv2/performance/ethanol_TBIL_29day_barplot_baseR.tif",
  width  = 5,
  height = 6,
  units  = "in",
  res    = 600,
...
# ---- Custom y-axis: ticks and labels up to 0.40 ----
axis_ticks <- seq(0, 0.4, by = 0.1)
axis(
  side   = 2,
  at     = axis_ticks,
