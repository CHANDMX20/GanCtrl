...
GROUPS = {
    ...
    # (rest not needed for inference, but harmless)
}
...
# ==============================
# Load composite model and build MeanPredictor + Sampler
# ==============================
STAGE_DIM = len(stageBinarizer.classes_)
BIO_DIM   = len(bioCopyBinarizer.classes_)
DOSE_DIM  = len(doseBinarizer.classes_)
BW_DIM    = 1
LABEL_DIM   = DOSE_DIM + STAGE_DIM + BIO_DIM + BW_DIM     # dose+time+bio+BW
TARGET_DIM  = STAGE_DIM + BIO_DIM + BW_DIM                # time+bio+BW
CLUSTER_DIM = clusterB.classes_.size

# ---- Set your checkpoint ----
MODEL_STEP = 929200# <- change to the step you saved

BASE_RESULTS = '/account001/mansi.chandra/clin_path/results_vae_corr_mod'  # match training save path

# Try to load the composite first (preferred: it preserves exact wiring)
try:
    ...
    enc = None  # we won't need it if we sample with a dummy z (not preferred)
    # Preferred: also save and load e_model.h5; if not available, use z=0.
...
def build_mean_predictor(enc_model, gen_model):
    ...
    li = Input((LABEL_DIM,),    name='mp_label_in')   # dose+time+bio+BW (src)
    ti = Input((TARGET_DIM,),   name='mp_target_in')  # time+bio+BW (tgt)
    ci = Input((CLUSTER_DIM,),  name='mp_cluster_in')

    if enc_model is not None:
        _, _, z = enc_model([fi, li, ti, ci])
    else:
        # Fallback: deterministic zero-z if Encoder wasnt loaded (keeps dims consistent)
        z = Lambda(lambda x: tf.zeros((tf.shape(x)[0], 64), dtype=tf.float32), name='mp_zero_z')(fi)
...
def build_sampler(enc_model, gen_model):
    ...
    if enc_model is not None:
        ...
        z_ = Lambda(lambda x: tf.zeros((tf.shape(x)[0], 64), dtype=tf.float32), name='sp_zero_z')(fi)
...
def summarize_means(
    ...
    out.to_csv(
        f"{out_dir}/generated_predictions_{step+1:06d}_{name}.csv",
        index=False
    )
...
def summarize_samples(
    ...
    for s in range(1, num_samples + 1):
        ...
        out.to_csv(
            f"{out_dir}/generated_samples_s{s}_{step+1:06d}_{name}.csv",
            index=False
        )
