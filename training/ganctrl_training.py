...
Trains a one-sided treatment → control translation model with:

  - Strict reproducibility for TensorFlow / NumPy / Python.
  - Data loading for control/treatment train & test splits (2D CSV layout).
  - BODY_WEIGHT merge (keep latest per PROGRESS_TIME per composite key).
  - MinMax scaling (per split) for *clinical parameter columns only*.
  - Per-sample peer variance maps (controlTrain, scaled space).
  - KMeans clustering + one-hot cluster encoding.
  - Feature weighting for key biomarkers in the NLL loss.
  - Molecular descriptors embedded inside the *encoder* (not used in generator).
  ...
  - CVAE-GAN:
        * Encoder (q_phi): includes mol embedding → z
        * Generator VAE (p_theta): baseline (NO mol input)
        * Conditional Discriminator
        * Composite model ties together KL, NLL, GAN, TBIL-range,
...
Intended usage:
  ...
  - Put files under ./data (or set env var GANCTRL_DATA_DIR).
  - Outputs written under ./results (or set env var GANCTRL_RESULTS_DIR).
"""
...
resultPath = os.environ.get("GANCTRL_RESULTS_DIR", "./results")
...
# =============================================================================
# Column slices (2D layout)
#   - mol descriptors: columns [11:PARAMS_START)
#   - clinical parameters/features: [PARAMS_START:]
#
# IMPORTANT:
#   - We MinMax scale ONLY the clinical parameter columns (`cols`).
#   - Molecular descriptors are scaled separately (StandardScaler) for encoder stability,
#     but are NOT used by the generator.
# =============================================================================
PARAMS_START = 1368
if len(dataset.columns) <= PARAMS_START:
    raise RuntimeError(
        f"Expected parameters to start at column index {PARAMS_START}, "
        f"but dataset has only {len(dataset.columns)} columns. Check your 2D CSV layout."
    )

MOL_COLS = list(dataset.columns[11:PARAMS_START])     # molecular descriptor columns
cols     = list(dataset.columns[PARAMS_START:])       # clinical parameter columns (model outputs)
A        = len(cols)
...
#   - keeps latest entry per composite key using PROGRESS_TIME
# =============================================================================
...
# =============================================================================
# Scaling (UNCHANGED behavior for clinical params):
#   - Fit MinMax on train split
#   - Transform the corresponding split(s)
#   - Only scales `cols` (clinical parameters)
# =============================================================================
...
# =============================================================================
# Molecular descriptor scaling (encoder stability only)
#   - does NOT affect generator input/architecture
#   - Fit on treatmentTrain mols; apply to all splits
# =============================================================================
...
# =============================================================================
# Real variance map (per-sample peer variance, computed from controlTrain)
#   - used as supervision target for the variance head via NLL regularization
# =============================================================================
...
# =============================================================================
# Clustering
#   - KMeans on controlTrain in parameter space (scaled)
#   - Map treatmentTrain clusters by compound-majority cluster assignment
#   - Assign treatmentTest clusters by nearest cluster mean, then per-compound majority
# =============================================================================
n_clusters = 6
...
# =============================================================================
# Feature weighting (for NLL / variance supervision)
# =============================================================================
feature_weights = {
    'ALP(IU/L)':       200.0,
    'TC(mg/dL)':       100.0,
...
# =============================================================================
# Spatial groups mask (biologically-informed mixing graph)
#   - builds a (A x A) neighbor mask for attention mixing
# =============================================================================
ALIASES = {
...
def _compute_group_minmax(ctrl_df, cols):
    """
    For each (COMPOUND_NAME, SACRIFICE_PERIOD) group in controlTrain:
      - store per-feature min/max (scaled space)
    Used for TBIL range loss bounds.
    """
    mm = {}
    grouped = ctrl_df.groupby(['COMPOUND_NAME', 'SACRIFICE_PERIOD'], dropna=False)
...
def _build_tbil_bounds_for_rows(c_rows):
    """
    Build [lo, hi] TBIL bounds per row based on its (compound, timepoint) control min/max.
    Falls back to global TBIL bounds if group is missing.
    """
...
# =============================================================================
# BIO MoE groups (feature-expert sets for correlation loss)
#   - defines expert feature subsets
#   - builds all pairwise index pairs within each subset
# =============================================================================
def _safe_idx(name):
...
for name, pairs in _EXPERT_PAIRS.items():
    if not pairs:
        continue
    I = [i for (i, _) in pairs]
    J = [j for (_, j) in pairs]
    W = [EXPERT_WEIGHTS.get(name, 1.0)] * len(pairs)
...
# =============================================================================
# Label dims (derived from fitted binarizers)
# =============================================================================
STAGE_DIM = len(stageBinarizer.classes_)
BIO_DIM   = len(bioCopyBinarizer.classes_)
DOSE_DIM  = len(doseBinarizer.classes_)
BW_DIM    = 1
LABEL_SHAPE  = (DOSE_DIM + STAGE_DIM + BIO_DIM + BW_DIM,)
TARGET_SHAPE = (STAGE_DIM + BIO_DIM + BW_DIM,)
...
# =============================================================================
# Variance-focused NLL (baseline)
# =============================================================================
def make_gaussian_nll(lambda_var=6.0, batch_var_weight=0.6, floor=-3.0, ceil=-1.5, var_cons_in_logspace=True):
...
# =============================================================================
# Encoder (UPDATED: mol embedding lives here; generator unchanged)
#   - mol descriptors influence z through a learned embedding
#   - z_logvar is clipped for stability (prevents exp overflow)
# =============================================================================
def define_encoder(input_shape, mol_shape, label_shape, target_shape, cluster_shape,
                  z_dim=64, mol_emb_dim=32, zlogvar_floor=-6.0, zlogvar_ceil=2.0):
    init = RandomNormal(stddev=0.01)
    f_in = Input(shape=input_shape,   name='enc_feat_in')
    m_in = Input(shape=mol_shape,     name='enc_mol_in')
    ...
    m = Dense(256, activation='relu', kernel_initializer=init, name='mol_d1')(m_in)
    m = Dense(128, activation='relu', kernel_initializer=init, name='mol_d2')(m)
    m_emb = Dense(mol_emb_dim, kernel_initializer=init, name='mol_emb')(m)

    x = Concatenate(name='enc_concat')([f_in, m_emb, l_in, t_in, c_in])
    ...
    z_logvar_raw = Dense(z_dim, kernel_initializer=init, name='z_logvar_raw')(x)
    z_logvar = Lambda(
        lambda lv, f=zlogvar_floor, c=zlogvar_ceil: tf.clip_by_value(lv, f, c),
        name='z_logvar'
    )(z_logvar_raw)

    def _sample(args):
        m_, lv = args
        eps = K.random_normal(shape=K.shape(m_))
        return m_ + K.exp(0.5 * lv) * eps

    z = Lambda(_sample, name='z_sample')([z_mean, z_logvar])
    return Model([f_in, m_in, l_in, t_in, c_in], [z_mean, z_logvar, z], name='Encoder')

# =============================================================================
# Generator VAE (BASELINE — unchanged; NO mol input)
# =============================================================================
...
# =============================================================================
# Composite CVAE-GAN (baseline weights; mol only feeds encoder)
#   - includes stability: clip generator logvar for sampling only
# =============================================================================
...
def define_composite_cvae_gan(e_model, g_model, d_model,
                              input_shape, mol_shape, label_shape, target_shape, cluster_shape,
                              beta_kl=0.05,
                              range_mu_weight=1.0, range_x_weight=2.0,
                              bio_corr_weight=5.0,
                              report_corr_metric=True,
                              mean_match_weight=0.05):
    d_model.trainable = False

    f_in   = Input(input_shape,   name='feat_in')
    m_in   = Input(mol_shape,     name='mol_in')          # encoder only
    l_in   = Input(label_shape,   name='label_in')
    t_in   = Input(target_shape,  name='target_in')
    c_in   = Input(cluster_shape, name='cluster_in')
    rng_tb = Input((2,),          name='tbil_range_in')
    corr_ref_in = Input(input_shape, name='corr_ref_in')

    z_mean, z_logvar, z = e_model([f_in, m_in, l_in, t_in, c_in])
    ...
    frac_at_floor = Lambda(
        lambda lv, f=FLOOR: tf.reduce_mean(tf.cast(tf.less_equal(lv, f + 1e-6), tf.float32)),
        name='frac_logvar_floor_value'
    )(logvar)
    frac_at_ceil = Lambda(
        lambda lv, c=CEIL: tf.reduce_mean(tf.cast(tf.greater_equal(lv, c - 1e-6), tf.float32)),
        name='frac_logvar_ceil_value'
    )(logvar)

    logvar_clip = Lambda(lambda lv, f=FLOOR, c=CEIL: tf.clip_by_value(lv, f, c),
                         name='logvar_clip_for_sampling')(logvar)

    def _sample_raw(args):
        mu01, lv = args
        eps = K.random_normal(shape=K.shape(mu01))
        return mu01 + K.exp(0.5 * lv) * eps
    x_raw = Lambda(_sample_raw, name='x_raw')([mu, logvar_clip])
    ...
    x_hat = Lambda(_sample_x, name='x_sample')([mu, logvar_clip])
    ...
    comp = Model([f_in, m_in, l_in, t_in, c_in, rng_tb, corr_ref_in],
    ...
    nll_loss_fn     = make_gaussian_nll(lambda_var=6.0,
...
#   - Mean predictor: returns mu only
#   - Sampler: returns sampled x (uses clipped logvar for sampling)
# =============================================================================
def build_mean_predictor(e_model, g_model, input_shape, mol_shape, label_shape, target_shape, cluster_shape):
    f_in = Input(input_shape)
    m_in = Input(mol_shape)
    l_in = Input(label_shape)
    t_in = Input(target_shape)
    c_in = Input(cluster_shape)
    _, _, z = e_model([f_in, m_in, l_in, t_in, c_in])
    mu_logvar = g_model([f_in, l_in, t_in, z, c_in])
    mu = Lambda(lambda y: y[:, :A], name='mu_out')(mu_logvar)
    return Model([f_in, m_in, l_in, t_in, c_in], mu, name='MeanPredictor')

def build_sampler(e_model, g_model, input_shape, mol_shape, label_shape, target_shape, cluster_shape,
                  floor=-3.0, ceil=-1.5):
    f_in = Input(input_shape)
    m_in = Input(mol_shape)
    l_in = Input(label_shape)
    ...
    logvar_clip = Lambda(lambda lv, f=floor, c=ceil: tf.clip_by_value(lv, f, c))(logvar)
    ...
    return Model([f_in, m_in, l_in, t_in, c_in], x_sample, name='Sampler')

# =============================================================================
# Pairing helpers (time-matched batches; returns mol batch too)
# =============================================================================
...
def sample_time_matched_batch_from_index(
    ...
    M_treat = t_rows[MOL_COLS].astype('float32').values
    ...
    return X_treat, M_treat, X_ctrl, real_var, L_src, L_tgt, C, y_real, TBIL_BOUNDS
...
def generate_test_real_samples(treatmentTest, controlTest):
    ...
    mol  = data[MOL_COLS].astype('float32').values
...
def generate_fake_samples(sampler_model, feats, mols, lbl_s, lbl_t, clusters):
    xs = sampler_model.predict([feats, mols, lbl_s, lbl_t, clusters], verbose=0)
...
#   - runs mean predictor (mu) on test-pair inputs
#   - inverse transforms with the provided scaler
#   - writes a CSV that includes meta columns + generated parameter values
# =============================================================================
...
# =============================================================================
# Training loop
#   - trains composite (G via c_model) each step
#   - then trains discriminator on real/fake samples
#   - periodically saves checkpoints + predictions + loss logs
# =============================================================================
def train_one_sided_translation(
    ...
    for i in range(start_step, total_steps):
        ...
        XrB, MrB, XrA, RVAR, LrB, LrA, ClrB, yrA_tf, TBIL_BOUNDS = sample_time_matched_batch_from_index(
        ...
        g_res = c_model.train_on_batch(
            [XrB, MrB, LrB, LrA, ClrB, TBIL_BOUNDS, XrA],
        ...
        Xf_s, y_fake = generate_fake_samples(sampler_model, XrB, MrB, LrB, LrA, ClrB)
...
# =============================================================================
# Build & Train (baseline generator; mol only in encoder)
# =============================================================================
input_shape    = (len(cols),)
mol_shape      = (len(MOL_COLS),)
...
e_model = define_encoder(
    input_shape, mol_shape, label_shape, target_shape, cluster_shape,
    z_dim=z_dim, mol_emb_dim=32,
    zlogvar_floor=-6.0, zlogvar_ceil=2.0
)
...
mean_predictor = build_mean_predictor(e_model, g_model, input_shape, mol_shape, label_shape, target_shape, cluster_shape)
sampler_model  = build_sampler(e_model, g_model, input_shape, mol_shape, label_shape, target_shape, cluster_shape,
                               floor=-3.0, ceil=-1.5)
