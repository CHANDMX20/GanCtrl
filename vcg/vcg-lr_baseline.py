"""Construct and evaluate lab-relaxed virtual control groups (VCG-LR).

VCG-LRs are sampled from training-set historical controls matched to each test
compound/time group by sacrifice time and vehicle. Laboratory is deliberately
not used as an inclusion criterion, allowing controls from the same or different
laboratories to contribute to the historical-control pool. Controls from the
target compound are excluded to prevent information leakage.

Each VCG-LR contains the same number of animals as the corresponding real
...
def generate_vcg_lr_zscores(
    historical_controls,
    real_controls,
    treatments,
    feature_cols,
):
    """Generate same-time, same-vehicle VCG-LRs and treatment z-scores."""
    ...
    for iteration in range(1, N_VCG_REPS + 1):
        for (compound, time), treatment_group in treatments.groupby(
            ...
            # Lab-relaxed historical-control pool:
            #   - same sacrifice time
            #   - same vehicle
            #   - any laboratory
            #   - different compound
            #
            # Laboratory is not filtered here. This allows the VCG-LR to draw
            # controls from the target laboratory and/or other laboratories.
            pool_mask = (
            ...
            # Sample a VCG-LR with the same number of animals as the real control group.
            ...
            if not vcg_sample.empty:
                ...
                sample_log["source_is_target_lab"] = sample_log["sampled_lab"].eq(
                    target_lab
                )
            ...
            # Score each high-dose animal relative to the sampled VCG-LR.
    ...
    results_vcg_lr_df = pd.DataFrame(vcg_rows)
    vcg_lr_group_log_df = pd.DataFrame(group_log_rows)
    vcg_lr_sample_log_df = (
        pd.concat(sample_log_rows, ignore_index=True)
        if sample_log_rows
        else pd.DataFrame()
    )

    return results_vcg_lr_df, vcg_lr_group_log_df, vcg_lr_sample_log_df
...
def evaluate_vcg_lr_concordance(real_calls_df, vcg_lr_calls_df):
    """Compare each VCG-LR iteration-level call with the corresponding real call."""
    merge_keys = ["compound", "time", "dose", "feature"]

    real_calls = real_calls_df[
        merge_keys + ["real_call"]
    ].copy()
    vcg_calls = vcg_lr_calls_df[
        ["vcg_iter"] + merge_keys + ["vcg_lr_call"]
    ].copy()

    merged = vcg_calls.merge(real_calls, on=merge_keys, how="inner")
    merged["real_abnormal"] = merged["real_call"].eq("abnormal")
    merged["vcg_abnormal"] = merged["vcg_lr_call"].eq("abnormal")
...
def main():
    """Run the complete VCG-LR construction and concordance workflow."""
    ...
    # -------------------------------------------------------------------------
    # VCG-LR construction and calls
    # -------------------------------------------------------------------------
    results_vcg_lr_df, vcg_lr_group_log_df, vcg_lr_sample_log_df = generate_vcg_lr_zscores(
    ...
    vcg_lr_calls_df = build_feature_calls(
        results_vcg_lr_df,
        keys=["vcg_iter", "compound", "time", "dose"],
        call_col="vcg_lr_call",
    )
    feature_status_vcg_lr_df = summarize_feature_calls(
        vcg_lr_calls_df,
        call_col="vcg_lr_call",
    ...
    # -------------------------------------------------------------------------
    # Concordance between VCG-LR-based and real-control-based calls
    # -------------------------------------------------------------------------
    (
        merged_vcg_lr_calls_df,
        confusion_vcg_lr_df,
        iteration_accuracy_vcg_lr_df,
        accuracy_summary_vcg_lr_df,
    ) = evaluate_vcg_lr_concordance(real_calls_df, vcg_lr_calls_df)
    ...
    results_vcg_lr_df.to_csv("vcg_lr_zscores.csv", index=False)
    vcg_lr_calls_df.to_csv("vcg_lr_feature_calls.csv", index=False)
    feature_status_vcg_lr_df.to_csv("vcg_lr_feature_status_by_iteration.csv", index=False)
    vcg_lr_group_log_df.to_csv("vcg_lr_group_log.csv", index=False)
    vcg_lr_sample_log_df.to_csv("vcg_lr_sample_log.csv", index=False)

    merged_vcg_lr_calls_df.to_csv("vcg_lr_merged_calls.csv", index=False)
    confusion_vcg_lr_df.to_csv("vcg_lr_concordance.csv", index=False)
    iteration_accuracy_vcg_lr_df.to_csv("vcg_lr_accuracy_by_iteration.csv", index=False)
    accuracy_summary_vcg_lr_df.to_csv("vcg_lr_accuracy_summary.csv", index=False)

    print("VCG-LR analysis complete.")
    print("\nPooled VCG-LR concordance by feature:")
    print(confusion_vcg_lr_df)
    print("\nMean accuracy across VCG-LR iterations:")
    print(accuracy_summary_vcg_lr_df)
